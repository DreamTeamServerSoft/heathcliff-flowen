...
cli.on("message", function (message) {
  if (message.content == "!role pingme") {
    ...
    message.channel.send(
      "You are now subscribed to DreamTeam announcements! You will be pinged by the 'Announcements' channel when something new comes to DreamTeam. You will also be notified if a YouTube video is posted on our official YouTube channel."
    );
  } else if (message.content == "!remove pingme") {
    message.member.roles.remove(pingme);
    message.channel.send(
      "You have unsubscribed from DreamTeam announcements. :("
    );
  }
});
...
cli.on("message", function (message) {
  if (message.content === "!flipcoin") {
    // Coin Flip Ary (Heads or Tails).
    const aryFlip = [
      "https://i.postimg.cc/Hkvw4ZqY/heads.png",
      "https://i.postimg.cc/kMZPzqWK/tails.png",
    ];

    // Generate Random Integers Between 1 & 2.
    let flip = aryFlip[Math.floor(Math.random() * aryFlip.length)];

    // Heathcliff's Epic Monolouge!
    setTimeout(() => {
      message.channel.send(
        "Alright suckers, it's time to play 'Coin Tawce!™'. If the coin lands on MY MUG (heads), you LOOSE!"
      );
    }, 1000);
    setTimeout(() => {
      message.channel.send(
        "On the other side, if you reveal a GAE BOLG (tails), sadly, you win."
      );
    }, 4000);
    setTimeout(() => {
      message.channel.send("Think you have what it takes? Let's do this.");
    }, 5400);
    setTimeout(() => {
      message.channel.send("- Flips Coin -");
    }, 6800);
    setTimeout(() => {
      // Display Head or Tail of 'Coin'.
      message.channel.send({ files: [flip] });
    }, 8200);
  }
});
...
cli.on("message", function (message) {
  if (message.content == "!role dcv1") {
    ...
    message.channel.send(
      "You have successfully removed 'PSO Dreamcast Version 1' from your Discord account."
    );
  }
});
...
cli.on("message", function (message) {
  if (message.content == "!role dcv2") {
    ...
    message.channel.send(
      "You have successfully removed 'PSO Dreamcast Version 2' from your Discord account."
    );
  }
});
...
cli.on("message", function (message) {
  if (message.content == "!role psogc") {
    ...
    message.channel.send(
      "You have successfully removed 'PSO GameCube' from your Discord account."
    );
  }
});
...
cli.on("message", function (message) {
  if (message.content == "!role quake3") {
    ...
    message.channel.send(
      "You have successfully removed 'Quake III Arena' from your Discord account."
    );
  }
});
...
cli.on("message", function (message) {
  if (message.content == "!role halo") {
    ...
    message.channel.send(
      "You have successfully removed 'Halo' from your Discord account."
    );
  }
});
